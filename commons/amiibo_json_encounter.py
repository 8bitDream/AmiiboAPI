...
class AmiiboJSONEncoder(DefaultJSONProvider):
    def default(self, obj):
        if isinstance(obj, Hex):
            ...
            image_name = "icon_{}-{}".format(str(obj.head)[2:], str(obj.tail)[2:])
            if obj.variant is not None:
                image_name = "{}_{}".format(image_name, obj.variant)
            returner = {
                'name': obj.name,
                'head': str(obj.head)[2:],
                'tail': str(obj.tail)[2:],
                ...
                'image': "https://raw.githubusercontent.com/8bitDream/AmiiboAPI/master/images/{}.png".format(image_name),
                'imgwebp': "https://raw.githubusercontent.com/8bitDream/AmiiboAPI/master/webp/{}.webp".format(image_name),
                'release': obj.release
            }
            if obj.variant is not None:
